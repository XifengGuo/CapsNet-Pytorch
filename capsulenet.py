...
class CapsuleNet(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.digitcaps(x)
        length = x.norm(dim=-1)
        if y is None: # during testing, no label given
...
def caps_loss(y_pred, y_true, x_recon, x):
    ...
    return L_margin + 0.0005 * L_recon
...
def test(model, test_loader):
    ...
    for x, y in test_loader:
        ...
        test_loss += caps_loss(y_pred, y, x_recon, x).data[0] * x.size(0) # sum up batch loss
...
def train(model, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            loss = caps_loss(y_pred, y, x_recon, x)
            loss.backward()
            training_loss += loss.data[0] * x.size(0)
            optimizer.step()
...
if __name__ == "__main__":
    ...
    if not args.testing:
        ...
        test_loss, test_acc = test(model=model, test_loader=test_loader)
